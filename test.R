...
#######################################################################
#  Data exploration
data_fin2 <- read.csv2(here("CBH_db_update2025.csv"))
...
ggplot(tmp_ageyr, aes(Year, mean_len,colour=as.factor(agePlus ))) + geom_point(size=1.3)  + geom_line(linewidth=0.5)  +theme(legend.position="rigth")+ theme_bw()#+ geom_smooth(se = F)
dev.off()
...
ggplot(tmp_area_ageyr, aes(Year, mean_len,colour=as.factor(agePlus ))) + geom_point(size=1.3)+ geom_line(linewidth=0.4)+ theme_bw() + facet_wrap(~Area) # + geom_smooth(se =F)
dev.off()
...
## Test
mod5 <- nlme(LENGTH ~ Linf*(1-exp(-k0*(AgeFraction-(t0))))  ,fixed= Linf+k0+t0 ~1 ,random=  list(Cohort = Linf+k0~1) ,
             data=data_fin2, start=c(Linf=259.7278,k0=0.1492918,t0=-3.639129)) 

mod6 <- nlme(LENGTH ~ Linf*(1-exp(-k0*(AgeFraction-(t0)))),
             fixed = Linf+k0+t0 ~1,
             random = Linf+k0~1|Cohort,
             data = data_fin2,
...
mod7 <- nlme(LENGTH ~ Linf*(1-exp(-k0*(AgeFraction-(t0)))),
             fixed= Linf+k0+t0 ~1,
             random = Linf+k0~1|Area/Cohort,
             data=data_fin2, start=c(Linf=259.7278,k0=0.1492918,t0=-3.639129)) 

summary(mod7)

# Now try of syntax
mod8 <- nlme(LENGTH ~ Linf*(1-exp(-k0*(AgeFraction-(t0)))),
             fixed = Linf + k0 + t0 ~1,
             random = list(Area = Linf + k0 ~ 1,
                           Cohort = Linf + k0 ~ 1),
             data = data_fin2,
             start = c(Linf = 259.7278, k0 = 0.1492918, t0 = -3.639129)) 
...
# https://stats.stackexchange.com/questions/648486/clarification-on-random-effects-structure-in-linear-mixed-models-in-r
